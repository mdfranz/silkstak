...
/// provider, then a built-in fallback. Returns the model string, or None if
/// `provider` is unknown and has no configured default.
pub(crate) fn default_model_for_provider(provider: &str, cfg: &Config) -> Option<String> {
